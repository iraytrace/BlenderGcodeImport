...
class OldParser():
    ...
    def __init__(self):
        ...
        self.filament_volume = 0
        self.heightOfLayer = 0
        self.width_of_layer = 0
    ...
    def parse(self, fileName):
        ...
        f0 = open(fileName)
        file_text = f0.read()
        filament_dia = self.findfilamentdia(file_text)
        self.filament_volume =((filament_dia / 2) ** 2) * 3.14
        f0.close()
    ...
    def build(self):
        ...
        profilePoly.points.add(3)
        profilePoly.points[3].co = (.5 * self.xOoze, 0.0,           0.0, 1)
        profilePoly.points[2].co = (.5 * self.xOoze, self.heightOfLayer,  0.0, 1)
        profilePoly.points[1].co = (-.5 * self.xOoze, self.heightOfLayer, 0.0, 1)
        profilePoly.points[0].co = (-.5 * self.xOoze, 0.0,          0.0, 1)
    ...
    def moveTo(self, newPos):
        if newPos['Z'] != self.pos['Z']:
            self.heightOfLayer = newPos['Z'] - self.pos['Z']
            if self.heightOfLayer <= 0:
                self.heightOfLayer = newPos['Z']
            self.newLayer()
        
        if newPos['E'] <= self.pos['E'] or newPos['E'] <= 0.0:
            self.newPoly()
            if newPos['E'] != self.pos['E']:
                self.pos['E'] = newPos['E']

        print( 'moveTo(%s %s %s %s %s)' % (newPos['X'], newPos['Y'], newPos['Z'], self.heightOfLayer, self.width_of_layer))

        #if plastic is extruded
        if newPos['E'] > 0 and newPos['E'] > self.pos['E']:
            length = ((newPos['Y'] - self.pos['Y'])**2 + (newPos['X'] - self.pos['X'])**2)**.5
            if length <= 0:
                print( 'zero length move')
                if self.pos['E'] != newPos['E']:
                    self.pos['E'] = newPos['E']
                return

            self.width_of_layer = (self.filament_volume * (newPos['E'] - self.pos['E'])) / (length * self.heightOfLayer)
            #saves new postion in the list of points where plastic is being placed
    ...
    def parseCoordsUpdate(self, tokens):
        ...
        return npos
    ##### FIND FILAMENT DIAMETER #####
    def findfilamentdia(self, file):
        filament_dia_index = int(file.find("; filament_diameter = ")) + 22
        boolean = 1
        filament_dia = ""
        while boolean:
            if file[filament_dia_index] == ";":
                boolean = 0
            else:
                filament_dia += file[filament_dia_index]
                filament_dia_index += 1
        return float(filament_dia.strip())
...
if __name__ == "__main__":
    parser = OldParser()
    parser.parse("C:/Users/Ken/Desktop/New Work/cube.gcode")
